 ...
 *
 * CFBD stamps each row with ESPN's own event id, so games join exactly by id
 * and no school-name matching is needed. The name key is kept as a fallback.
 *
 * @returns {Promise<Map<string, object>>} keyed by ESPN event id AND by
...
export async function getSeasonLines(season, key, seasonType = 'regular') {
  ...
  for (const g of rows || []) {
    ...
    if (g.id != null) out.set(String(g.id), entry);
    out.set(`${normalizeTeam(g.awayTeam)}|${normalizeTeam(g.homeTeam)}`, entry);
  }
  return out;
}
