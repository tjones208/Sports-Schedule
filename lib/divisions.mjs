...
// Keyed by season: conference membership changes year to year, and a cache
// that ignored the season would score a 2025 backtest against the 2026 roster.
const cache = new Map();   // season -> { ids, at }
...
export async function getFbsTeamIds(season = new Date().getUTCFullYear()) {
  ...
  try {
    ...
    return hit ? hit.ids : null;   // fail open rather than empty the board
  }
}
